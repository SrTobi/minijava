...
BOOST_AUTO_TEST_CASE(token_ctor_id)
{
	...
	const auto canonical = minijava::string::create_from_canonical_pointer(name.c_str());
...
BOOST_AUTO_TEST_CASE(identifiers_with_different_name_compare_not_equal)
{
	...
	const auto canon1 = minijava::string::create_from_canonical_pointer(name1.c_str());
	const auto canon2 = minijava::string::create_from_canonical_pointer(name2.c_str());
